...
describe("env:set", () => {
  ...
  test
    .stdout()
    .command(["env:set", "./tmp/testenvironment.json", "--config=./tmp/testconfig.json"])
    .it("should overwrite existing config", (ctx) => {
      ...
      expect(config).to.contain("\"selectedEnvironment\":\"./tmp/testenvironment.json\"");
    });

  test
    .stdout()
    .command(["env:set", "./tmp/testenvironment.json", "--config=./tmp/testconfig-2.json"])
