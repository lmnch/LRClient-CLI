...
export class Contains<V> implements CallAssertion<{[key: string]: V}> {
  ...
  matches(param: {[key: string]: V}): boolean {
    return Object.entries(param).some(([key, value]) => this.#keyMatcher.matches(key) && this.#valueMatcher.matches(value))
  }
}
...
export class FetchCallAssertions implements CallAssertion<[RequestInfo|URL, RequestInit?]>{

  resource: CallAssertion<RequestInfo|URL> = new Any();
  ...
  matches([resource, reqInit]: [RequestInfo|URL, RequestInit?]) : boolean {
    ...
    const [_, queryParamString] = resource.toString().split("?");
...
const fetchMock = (callAssertions: FetchCallAssertions, response: Response) => {
  ...
  return (resource: RequestInfo|URL, init: RequestInit|undefined) : Promise<Response>|undefined => {
...
// TODO: do this better
const registeredMocks : ((resource: RequestInfo|URL, options: RequestInit|undefined)=>Promise<Response>|undefined)[] = [];

export function mock(requestParams : FetchCallAssertions = new FetchCallAssertions, response: Response) {
...
global.fetch = (resource: RequestInfo|URL, init: RequestInit|undefined) : Promise<Response> => {
  ...
  throw Error("Resource not mocked!")
}
